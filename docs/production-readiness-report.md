...
*Fix — partially done.* `runReminderSweep` (`Reminders.gs`) now skips any `AbandonedCarts`/`Orders` row older than `REMINDER_LOOKBACK_MS` (48 hours, defined as `48 * REMINDER_DELAY_MS`) before doing any per-row work, so the sweep's per-row cost (JSON parsing, catalog lookups, email formatting) no longer grows with total historical volume. This does **not** fix the underlying cost described above: `sheetToObjects()` has no server-side filter (Finding 2), so the full historical `Orders`/`AbandonedCarts` table is still read into memory in full every hour regardless of the lookback window — only the work *after* that read is now bounded. Archiving/exporting old rows out of the live sheet, which would bound the read itself, remains undone and out of scope for this pass.
