...
**Finding 7 — No archiving/retention on any append-only table.** *(Fixed, opt-in — see below. `Messages`/`TwoFACodes` are separate: `Messages` needs Finding 1's migration, `Sessions` pruning already shipped under Finding 5.)*
`Orders`, `Messages` (Finding 1), `AbandonedCarts`, and `TwoFACodes`/`Sessions` (partially, see Finding 5) only ever grow. `runReminderSweep` (`Reminders.gs:79`, runs hourly forever per the README's own setup instructions) scanned the **entire historical `Orders` and `AbandonedCarts` tables every hour**, for the lifetime of the marketplace, even though only rows from the last ~1-2 hours are ever actually due for a reminder. Cost grows linearly with total historical order volume, forever, with no time-bounded window.

*Fix — done in two layers.* First, `runReminderSweep` skips any `AbandonedCarts`/`Orders` row older than `REMINDER_LOOKBACK_MS` (48 hours) before doing any per-row work — bounds the sweep's per-row cost, but not the underlying read (`sheetToObjects()` has no server-side filter, per Finding 2 — the full historical table is still read into memory every hour regardless). Second, and closing the gap that layer left open: `Reminders.gs` gained `archiveOldOrders()`/`deleteStaleAbandonedCarts()`, also wired into the same hourly sweep. Orders older than ~12 months move into a new `OrdersArchive` tab and are deleted from the live `Orders` sheet — the live sheet's read cost (this sweep, `actionListOwnerOrders`, `actionUpdateOrderStatus`) now stays bounded by *recent* order volume, not total historical volume, closing the actual gap the first layer couldn't. Abandoned carts past the same window are deleted outright (no archive needed — nothing ever displays historical cart data to anyone). Archiving is opt-in via the `OrdersArchive` tab's existence (I can't create Sheet tabs myself), mirroring the Cloudinary/Resend opt-in shape elsewhere in this report — a deployment that never creates the tab keeps behaving exactly as before. An archived order disappears from the vendor dashboard by design (confirmed with the user), recoverable only via direct Sheet access — see `README.md`'s new "Order archiving" section. 175/175 backend tests passing (7 new).

**Finding 8 — Product/logo images were hotlinked from Google Drive, not served from a CDN.** *(Fixed, opt-in — see below.)*
`Images.gs` and `Chat.gs`'s chat-image upload both served images via `lh3.googleusercontent.com/d/<fileId>` links. This works but Drive is a file-storage product, not a CDN — it carries its own per-file download-quota behavior under heavy simultaneous access and has no SLA for high-traffic hotlinking. At 100,000 customers browsing product photos, this is a real availability risk under load, distinct from and in addition to the Sheets/Apps Script constraints above.
...
7. **Opt-in Orders/AbandonedCarts archiving** (`Reminders.gs`'s `archiveOldOrders`/`deleteStaleAbandonedCarts`) — closes the rest of Finding 7, opt-in via a new `OrdersArchive` Sheet tab's existence (same shape as the Cloudinary/Resend Script-Property opt-ins, just gated on a tab instead). Retention window, archive-vs-delete-vs-external-export, and vendor-visibility-after-archiving were all confirmed with the user (AskUserQuestion) before implementation, since each is a real behavior decision, not just a technical one.

Every change above is `node --check`-clean; the full backend suite currently stands at 175/175 (up from a 147-test baseline before this review began). Each was verified in isolation at the time it landed — see each fix's own commit for its exact before/after test counts.
...
2. Move the Owners/Products/Orders read path off live Sheets scans onto an indexed store, or accept Sheets for those (they're much smaller than Messages, and ~~archiving for `Orders`/`AbandonedCarts`~~ is done, opt-in, per Finding 7 — this bounds the *live* sheet's growth once opted into, though a single sweep's read cost within the retention window is still an uncached full scan).
